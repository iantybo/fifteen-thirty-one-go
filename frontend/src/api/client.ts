...
export const api = {
  async register(req: RegisterRequest) {
    ...
    // Cache only non-sensitive profile fields for UI purposes
  ...
  async me() {
    // Always validate session with server to ensure security
    const res = await apiFetch<{ user: User }>(`${apiBaseUrl()}/api/auth/me`)
    if (!res) throw new ApiError('Unexpected empty response', UNEXPECTED_EMPTY_RESPONSE_STATUS)
    // Cache for UI fallback only, but always reconcile with server
    localStorage.setItem('cached_user', JSON.stringify(res.user))
    return res
  },
  async logout() {
