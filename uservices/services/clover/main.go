// Command clover is the "clover" microservice: it combines nobs and fifteens for edge-case detection.
//
...
// scaffold and cribbage domain library and calls peer "rockies" and "sage" over HTTP via the shared mesh client.
package main
...
func handleScore(svc *service.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.Mesh.CallAll(ctx,
			meshclient.Fanout{Peer: "rockies", Path: "/score", Req: hand, Out: &part0},
			meshclient.Fanout{Peer: "sage", Path: "/score", Req: hand, Out: &part1},
		); err != nil {
