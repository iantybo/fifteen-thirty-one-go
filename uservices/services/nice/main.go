// Command nice is the "nice" microservice: it combines fifteens and runs for low-card-heavy hands.
//
...
// scaffold and cribbage domain library and calls peer "sage" and "astros" over HTTP via the shared mesh client.
package main
...
func handleScore(svc *service.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.Mesh.CallAll(ctx,
			meshclient.Fanout{Peer: "sage", Path: "/score", Req: hand, Out: &part0},
			meshclient.Fanout{Peer: "astros", Path: "/score", Req: hand, Out: &part1},
		); err != nil {
