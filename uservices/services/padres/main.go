// Command padres is the "padres" microservice: it combines run and flush scoring into a sequence subtotal.
//
...
// scaffold and cribbage domain library and calls peer "astros" and "dodgers" over HTTP via the shared mesh client.
package main
...
func handleScore(svc *service.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.Mesh.Call(ctx, "astros", "/score", hand, &part0); err != nil {
		...
		if err := svc.Mesh.Call(ctx, "dodgers", "/score", hand, &part1); err != nil {
			service.WriteJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
			return
		}
