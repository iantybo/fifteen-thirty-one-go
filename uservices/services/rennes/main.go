// Command rennes is the "rennes" microservice: it combines runs and nobs to weigh the starter's impact.
//
...
// scaffold and cribbage domain library and calls peer "astros" and "rockies" over HTTP via the shared mesh client.
package main
...
func handleScore(svc *service.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := svc.Mesh.CallAll(ctx,
			meshclient.Fanout{Peer: "astros", Path: "/score", Req: hand, Out: &part0},
			meshclient.Fanout{Peer: "rockies", Path: "/score", Req: hand, Out: &part1},
		); err != nil {
